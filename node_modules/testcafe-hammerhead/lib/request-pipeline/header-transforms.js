...
function transformCookieForFetch(src, ctx) {
    const requestCredentials = ctx.req.headers[_headers2.default.fetchRequestCredentials];
...
}, _authorization2.default.headers.reduce((obj, header) => {
...
function transformHeaders(srcHeaders, ctx, transformList, forced) {
    ...
    if (forced) Object.keys(forced).forEach(headerName => applyTransform(headerName, destHeaders, forced));
