...
const IFRAME_FLAG_TAGS = _attributes.TARGET_ATTR_TAGS.target.filter(tagName => tagName !== 'base').concat(_attributes.TARGET_ATTR_TAGS.formtarget);
...
class DomProcessor {
    ...
    processElement(el, urlReplacer) {
        ...
        for (let i = 0; i < this.elementProcessorPatterns.length; i++) {
            ...
            if (pattern.selector(el) && !this._isShadowElement(el)) {
                for (let j = 0; j < pattern.elementProcessors.length; j++) pattern.elementProcessors[j].call(this, el, urlReplacer, pattern);
    ...
    _processRelPrefetch(el, urlReplacer, pattern) {
        ...
        if (attrValue) {
            const formatedValue = (0, _stringTrim2.default)(attrValue.toLowerCase());
